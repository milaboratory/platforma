...
export function getPath(
    ...
    if (options && !options.crossplatform) {
        return path.resolve(options.packageRoot, `pkg-${packageName}-${options.packageVersion}-${options.os}-${options.arch}.${options.ext}`)
    }

    return path.resolve(options.packageRoot, `pkg-${packageName}-${options.packageVersion}.${options.ext}`)
}

export async function create(logger: winston.Logger, contentRoot: string, dstArchivePath: string) : Promise<void> {
    ...
    if (format == '') {
        throw new Error(`Archive ${dstArchivePath} has unsupported extension. Cannot create archive of unknown format`)
    }
