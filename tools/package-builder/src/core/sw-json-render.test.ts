...
import { descriptorFilePath, readDescriptorFile } from "./resolver";
import { SwJsonRenderer, writeBuiltArtifactInfo } from "./sw-json-render";
...
describe("Renderer tests", () => {
  ...
  test("render docker with binary", () => {
    ...
    const eps = new Map([
      [javaDockerEpName, i.getMainEntrypoint(javaDockerEpName)],
      [javaEpName, i.getMainEntrypoint(javaEpName)],
    ]);
    ...
    const expectedTag = new RegExp(
      `${defaults.DOCKER_REGISTRY}:${test_assets.PackageNameNoAt}\\.${javaEpName}\\.(?<hash>.*)`,
    );
  ...
  test("render docker indinvidual entrypoint", () => {
    ...
    const expectedTag = new RegExp(
      `${defaults.DOCKER_REGISTRY}:${test_assets.PackageNameNoAt}\\.${dockerEpName}\\.(?<hash>.*)`,
    );
...
function writeTestArtifactInfo(
  ...
  writeBuiltArtifactInfo(artInfoPath, {
    ...
    remoteArtifactLocation:
      artifactType === "docker"
        ? docker.generateRemoteTagName(i.artifactName(artifact), "beefface")
        : i.artifactArchiveAddressPattern(artifact),
    uploadPath:
      artifactType === "docker"
        ? undefined
        : i.artifactArchiveFullName(artifact, util.currentPlatform()),
  });
}
