...
const createRunEnvironmentSwJson = (
  packageRoot: string,
  runenvArtifactID: string,
...
describe("Docker Python Functions", () => {
  ...
  beforeEach(() => {
    ...
    createRunEnvironmentSwJson(
      testPackageRoot,
      "@platforma-open/milaboratories.runenv-python-3:3.12.10",
      "3.12.10",
      [],
    );
  ...
  describe("prepareDockerOptions", () => {
    ...
    it("should use default Python version when environment has no version", () => {
      createRunEnvironmentSwJson(
        testPackageRoot,
        "@platforma-open/milaboratories.runenv-python-3:aaa",
        "",
        [],
      );
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "No Python version found in environment, using default: python:3.12-slim",
      );
...
describe("getPythonVersionFromEnvironment", () => {
  ...
  const dockerSafeCases: [string, string, string][] = [
    ...
    [
      "@platforma-open/milaboratories.runenv-python-3:3.13.7-windowsservercore-ltsc2025",
      "",
      "3.13.7-windowsservercore-ltsc2025",
    ],
  ];
  ...
  describe("without normalizeForDocker", () => {
    rawCases.forEach(([input, pyVersion, expected]) => {
      it(`should extract "${expected}" from "${input}"`, () => {
        ...
        expect(
          getRunEnvironmentPythonInfo(mockLogger, testPackageRoot, "test-package", input, {
            normalizeForDocker: false,
          }),
        ).toStrictEqual({
