...
import {
  Renderer,
  listSoftwareNames as listSoftwareEntrypoints,
...
export class Core {
  ...
  public buildPackage(
    ...
    if (!pkg.isBuildable) {
      ...
      this.logger.error(
        `  no 'binary' settings found: software '${pkg.id}' archive build is impossible for configuration inside '${util.softwareConfigName}'`
      );
