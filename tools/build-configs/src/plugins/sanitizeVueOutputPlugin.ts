...
/**
 * Sanitize Rolldown vue virtual module suffixes while preserving .vue
 * in main SFC filenames for correct TypeScript declaration resolution.
 *
 * Virtual modules (script/style/template blocks) get clean short names:
 *   "PlIcon24_vue_vue_type_script_setup_true_lang.js"  → "PlIcon24.vue2.js"
 *   "PlTooltip.vue_vue_type_style_index_0_lang.module.js" → "PlTooltip.style.js"
 *
 * Main SFC entries keep their .vue extension:
 *   "PlIcon24.vue.js" → "PlIcon24.vue.js" (unchanged)
 */
function sanitizeVueName(name: string): string {
  // Extract and preserve the final file extension (e.g. .js, .css, .d.ts, .d.ts.map)
  const extMatch = name.match(/(\.d\.ts(?:\.map)?|\.[a-z]+(?:\.map)?)$/);
  const ext = extMatch?.[0] ?? "";
  const base = ext ? name.slice(0, -ext.length) : name;
  // Remove ?query params
  let sanitized = base.replace(/\?[^/]*/g, "");
  // Strip Rolldown-converted vue virtual module suffixes
  // e.g. "PlIcon24_vue_vue_type_script_setup_true_lang" → "PlIcon24.vue2"
  // e.g. "PlTooltip.vue_vue_type_style_index_0_lang.module" → "PlTooltip.style"
  ...
  // Main SFC entries: .vue is preserved intentionally so that
  // TypeScript resolves Component.vue.d.ts from `import ... from './Component.vue'`
