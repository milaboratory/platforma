...
- 46d96ff: `structure refresh` now bootstraps a missing `kind/` package instead of failing.
...
- 976be34: Stop stripping the `mark-stable` script from the block facade on `structure refresh`.
...
- 9d67cc4: Fix `structure init` so a fresh block installs, passes `pnpm run upgrade-sdk`, packs, and releases at 1.0.0. The catalog now carries `typescript` and `@platforma-sdk/block-kind`, the seeded model no longer declares an unused parameter, every module carries a version (`pnpm pack` cannot resolve a `workspace:*` dep without one) — backfilled by a `structure refresh` too, so a block scaffolded earlier stops failing `do-pack`, and the root gets a `.changeset/` — the config `changeset version` needs, plus an empty changeset so the first release publishes 1.0.0 instead of 1.0.1.
...
- 3280c4e: Add `@platforma-sdk/block-kind` to the SDK catalog membership list.
...
- fb64329: Structurer: the generated block CI (`build.yaml`) now sets `require-package-path-bump: true`, so a block PR must bump the published `block` package with a changeset (an empty changeset or the `skip-changelog` label waives it). Requires the `require-package-path-bump` input to be live on the pinned `@v4` reusable workflow before blocks are refreshed.
...
- a9d3b89: Structurer: make `turbo.json` a managed file and ignore `.test_auth.json` in
...
- 00f9501: Run the generated block `build.yaml` workflow on the Hetzner CI runner.
...
- 3d4fe94: Fix the generated build workflow calling a removed script. The root
