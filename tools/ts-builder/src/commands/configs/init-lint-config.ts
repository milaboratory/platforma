...
const VALID_LINT_CONFIGS: OxlintConfigType[] = ['block-model', 'block-ui', 'block-test', 'test', 'node', 'browser'];

export const initLintConfigCommand = new Command('init-lint-config')
  .description('Initialize .oxlintrc.json config file')
  .option('--target <target>', 'Target type to infer lint config (node|browser|block-model|block-ui|block-test)')
