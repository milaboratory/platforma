...
export function downloadArchive(logger: winston.Logger, options?: {
    version?: string
...
export function extractArchive(logger: winston.Logger, options?: {
    version?: string,
    archivePath?: string,
    extractTo?: string,
}): string {
    ...
    if (!fs.existsSync(archivePath)) {
        throw new Error(`Platforma Backend binary archive not found at '${archivePath}'`);
    }
...
export function getBinary(
    logger: winston.Logger, options?: { version?: string, showProgress?: boolean }
): Promise<string> {
    return downloadArchive(logger, options).
        then((archivePath) => extractArchive(logger, { archivePath }))
}
