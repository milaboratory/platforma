  ...
  <Layout>
    <Split name="Text Field" :hide-second="true">
      ...
      <PlTextField
        ...
        label="Validation"
        :rules="[
          (v) => v.length > 5 || 'Should be over 5 characters',
          (v) => v.length > 10 || 'Should be over 10 characters',
        ]"
      />
      ...
      <PlTextField
        v-model="data.text"
        label="With fixed prefix & clearable"
        prefix="Data /&nbsp;"
        clearable
      />
      <PlTextField
        v-model="data.text"
        :label="data.text"
        placeholder="In label will be the value from the previous input"
      />
