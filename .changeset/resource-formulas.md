...
- The formula is CID-transparent — adding or changing a `memFormula`/`cpuFormula` does not change exec deduplication. (The formula AST rides a meta input; line-count metrics are derived from the input files, so they add no differentiation either.)
