...
`exec.builder` now fails with a clear, actionable error when a GPU is requested (`gpuMemory()` / `gpuFormula()`) on a backend with no GPU support (`exec.hasGpu` is false) instead of surfacing an opaque downstream failure. Blocks that can run on CPU should gate the request: `if exec.hasGpu { builder.gpuMemory(...) }`.
