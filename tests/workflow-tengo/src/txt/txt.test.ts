import type { ImportFileHandle, MiddleLayerDriverKit } from "@milaboratories/pl-middle-layer";
...
tplTest.concurrent.for(cases)(
  ...
  async ({ handleProvider, headOptions, expectedContent }, { helper, expect, driverKit }) => {
    ...
    const result = await helper.renderTemplate(false, "txt.head", ["result", "progress"], (tx) => ({
      importHandle: tx.createValue(Pl.JsonObject, JSON.stringify(importHandle)),
      headOptions: tx.createValue(Pl.JsonObject, JSON.stringify(headOptions)),
    }));
  ...
  async ({ helper, expect, driverKit }) => {
    ...
    await expect(async () => {
      const result = await helper.renderTemplate(
        ...
        (tx) => ({
          importHandle: tx.createValue(Pl.JsonObject, JSON.stringify(importHandle)),
          headOptions: tx.createValue(Pl.JsonObject, JSON.stringify({ lines: 20, maxBytes: 50 })),
        }),
      );
      const progress = result
        .computeOutput("progress", (a, ctx) => {
