import type { AnyFieldRef, ResourceId } from "@milaboratories/pl-middle-layer";
...
tplTest.concurrent("test waiter 1", async ({ pl, helper, expect }) => {
  ...
  const result = await helper.renderTemplate(
    ...
    async (tx) => {
      ...
      waitResource = await toGlobalResourceId(tx.createStruct(resourceType("TestWait", "1")));
  ...
  await pl.withWriteTx("Test", async (tx) => {
    tx.setField(field(waitResource, "nestedField"), tx.createValue(Pl.JsonObject, "{}"));
    await tx.commit();
  });
  await outputResult.refreshState();
...
tplTest.concurrent("test waiter 2", async ({ pl, helper, expect }) => {
  ...
  const result = await helper.renderTemplate(
    ...
    ["output"],
    async (tx) => {
      const waitResource = await toGlobalResourceId(tx.createStruct(resourceType("TestWait", "1")));
      tx.lockInputs(waitResource);
  ...
  await pl.withWriteTx("CreatePass", async (tx) => {
    passResource = await toGlobalResourceId(tx.createEphemeral(resourceType("TestWait", "1")));
