...
tplTest.concurrent(
  ...
  async ({ helper, expect }) => {
    const wf1 = await helper.renderWorkflow('workflow.exports.wf1', false, {
      a: 'a',
      b: 'b',
    });
    ...
    const wf2 = await helper.renderWorkflow('workflow.exports.wf2', false, {}, { parent: ctx });
...
tplTest.concurrent(
  ...
  async ({ helper, expect }) => {
    ...
    const wf4 = await helper.renderWorkflow('workflow.exports.wf4', false, {}, { parent: ctx, blockId: 'b2' });
...
tplTest.concurrent(
  ...
  // it may take long for test to complete.
  async ({ helper, expect }) => {
    const wf2 = await helper.renderWorkflow('workflow.exports.wf3', false, {});
