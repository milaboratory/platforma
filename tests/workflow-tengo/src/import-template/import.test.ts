...
tplTest.concurrent("test import template in pure template", async ({ helper, expect }) => {
...
tplTest.concurrent("test import template in workflow", async ({ helper, expect }) => {
  const wf = await helper.renderWorkflow("import-template.workflow", false, { a: "c", b: "d" });
