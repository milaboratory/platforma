...
const jsonFromBlob = (b: { content: Uint8Array }) => JSON.parse(Buffer.from(b.content).toString());
...
tplTest.concurrent.for([
  ...
  async ({ indices, expectedResult, nested, base }, { helper, expect }) => {
    const result = await helper.renderTemplate(true, "pframes.ll.agg_1", ["result"], (tx) => {
...
eTplTest.concurrent(
  ...
  async ({ helper, expect, stHelper }) => {
    ...
    const result = await helper.renderTemplate(true, "pframes.ll.agg_2", ["result"], (tx) => {
