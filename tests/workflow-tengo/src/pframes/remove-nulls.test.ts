...
tplTest.concurrent.for([
  ...
  async ({ isEph }, { helper, expect }) => {
    const result = await helper.renderTemplate(
      ...
      (tx) => {
        ...
        tx.createField(
          field(inputMap, "[1]"),
          "Input",
          tx.createValue(Pl.JsonObject, JSON.stringify({ a: 1 })),
        );
        tx.createField(field(inputMap, "[2]"), "Input", tx.createValue(Pl.RNull, ""));
        tx.createField(
          field(inputMap, "[3]"),
          "Input",
          tx.createValue(Pl.JsonObject, JSON.stringify({ a: 3 })),
        );
    ...
    const resultMapLength = await awaitStableState(
      result.computeOutput("result", (a) => {
  ...
  async ({ isEph }, { helper, pl, expect }) => {
    ...
    const resultMapLength = await awaitStableState(
      result.computeOutput("result", (a) => {
        ...
        return {
          type: a?.resourceType,
          data: Object.fromEntries(fields?.map((f) => [f, a?.traverse(f)?.getDataAsJson()]) ?? []),
        };
      }),
    );
    expect(resultMapLength).toStrictEqual({
