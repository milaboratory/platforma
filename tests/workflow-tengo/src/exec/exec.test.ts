 ...
 */
tplTest.concurrent("run-hello-world-go", async ({ helper, expect }) => {
  const helloText = "Hello from go!";

  const result = await helper.renderTemplate(false, "exec.run.hello_go", ["main"], (tx) => ({
 ...
 */
tplTest.concurrent("check-docker-cmd-preserved", async ({ helper, expect }) => {
  const result = await helper.renderTemplate(
    false,
    "exec.run.docker_constraints",
    ["main"],
...
tplTest.concurrent("run-empty-conda-env", async ({ helper, expect }) => {
  const helloText = "2.3.2";

  const result = await helper.renderTemplate(false, "exec.run.conda_empty_run", ["main"], (tx) => ({
    text: tx.createValue(Pl.JsonObject, JSON.stringify(helloText)),
  }));
  const mainResult = result.computeOutput("main", (a) => a?.getDataAsString());
...
tplTest.concurrent(
  ...
  async ({ helper, expect }) => {
    ...
    const result = await helper.renderTemplate(false, "exec.run.echo_to_value", ["main"], (tx) => ({
      text: tx.createValue(Pl.JsonObject, JSON.stringify(helloText)),
    }));
    const mainResult = result.computeOutput("main", (a) => a?.getDataAsString());
...
tplTest.concurrent("should save file set by regex", async ({ driverKit, helper, expect }) => {
  const result = await helper.renderTemplate(
    false,
    "exec.run.save_file_set",
    ["p", "x", "all"],
    (tx) => ({}),
  );

  const p = await result.computeOutput("p", (p) => p?.listInputFields()).awaitStableValue();
  const x = await result.computeOutput("x", (x) => x?.listInputFields()).awaitStableValue();
  const all = await result.computeOutput("all", (all) => all?.listInputFields()).awaitStableValue();
...
tplTest.concurrent("should run workdir processor", async ({ driverKit, helper, expect }) => {
  const result = await helper.renderTemplate(
    false,
    "exec.run.with_wd_processor",
    ["p"],
    (tx) => ({}),
  );
