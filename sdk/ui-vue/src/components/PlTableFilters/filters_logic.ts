...
export function getColumnName(column: PTableColumnSpec, index: string | number) {
  return (
    readAnnotation(column.spec, Annotation.Label)?.trim() ??
    "Unlabeled " + column.type + " " + index.toString()
  );
}
...
export function getFilterDefault(
  type: PlTableFilterType,
  reference?: undefined | number | string,
): PlTableFilter {
  switch (type) {
    case "isNotNA":
    case "isNA":
...
export function parseNumber(column: PTableColumnSpec, value: string): number {
  ...
  if ((type === "Int" || type === "Long") && !Number.isInteger(parsed))
    throw Error("Model value is not an integer.");
...
export function parseString(column: PTableColumnSpec, value: string): string {
  const alphabet =
    readDomain(column.spec, Domain.Alphabet) ?? readAnnotation(column.spec, Annotation.Alphabet);
  if (alphabet === "nucleotide" && !/^[AaTtGgCcNn]+$/.test(value))
    throw Error("Model value is not a nucleotide.");
  if (alphabet === "aminoacid" && !/^[AaCcDdEeFfGgHhIiKkLlMmNnPpQqRrSsTtVvWwYyXx*_]+$/.test(value))
    throw Error("Model value is not an aminoacid.");
  return value;
}
...
export function makeDiscreteOptions(column: PTableColumnSpec): ListOption<number | string>[] {
  ...
  try {
    ...
    console.error(
      `Column ${column.id} has invalid '${Annotation.DiscreteValues}' annotation: '${discreteValuesAnnotation}'`,
      err,
    );
    return [];
  }
...
export function isAlphabetic(column: PTableColumnSpec): boolean {
  return (
    ...
    (readDomain(column.spec, Domain.Alphabet) ??
      readAnnotation(column.spec, Annotation.Alphabet)) !== undefined
  );
}
