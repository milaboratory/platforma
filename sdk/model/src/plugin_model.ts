 ...
 */
export class PluginModelFactory<Data, Params, Config, Outputs> {
  private readonly name: string;
  private readonly dataModelFactory: (config: Config) => DataModel<Data>;
  private readonly outputs: { [K in keyof Outputs]: (ctx: PluginRenderCtx<Data, Params>) => Outputs[K] };
 ...
 *   .build();
 */
export class PluginModelBuilder<
  Data,
  Params = undefined,
  Config = undefined,
...
> {
  ...
  private constructor(input: {
    ...
    this.name = input.name;
    this.dataModelFactory = input.dataModelFactory;
    this.outputs = input.outputs ?? ({} as { [K in keyof Outputs]: (ctx: PluginRenderCtx<Data, Params>) => Outputs[K] });
  }
  ...
  output<const Key extends string, T>(
    ...
    return new PluginModelBuilder<Data, Params, Config, Outputs & { [K in Key]: T }>({
      ...
      outputs: {
        ...this.outputs,
        [key]: fn,
      } as { [K in keyof (Outputs & { [P in Key]: T })]: (ctx: PluginRenderCtx<Data, Params>) => (Outputs & { [P in Key]: T })[K] },
    });
  }
