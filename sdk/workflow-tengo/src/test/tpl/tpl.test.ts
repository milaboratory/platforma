...
tplTest("test template with maps output", async ({ helper, expect }) => {
  ...
  const simpleMap = (await result.computeOutput("simpleMap", (a) => a?.getDataAsJson()).awaitStableValue()) as Record<string, string>
  expect(simpleMap["a"]).eq("a");

  const strictMap = (await result.computeOutput("strictMap", (a) => a?.getDataAsJson()).awaitStableValue()) as Record<string, string>
