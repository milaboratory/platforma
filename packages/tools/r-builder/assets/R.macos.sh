...
if [ -z "${R_HOME_DIR}" ]; then
  echo "'R_HOME_DIR' environment variable is not set. R cannot work not knowing where its root"
  exit 1
fi
...
RHOME="${R_HOME_DIR}" # this is for Rscript. It checks RHOME instead of R_HOME in some versions
export RHOME
R_HOME="${R_HOME_DIR}"
export R_HOME
