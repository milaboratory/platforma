...
eTplTest.concurrent(
  ...
  async ({ helper, expect, stHelper }) => {
    const result = await helper.renderTemplate(true, "pframes.proc_1", ["result"], (tx) => {
    ...
    expect(bSpec.axesSpec).toMatchObject([...inputSpec.axesSpec, xsvSettings.axes[0].spec]);
...
eTplTest.concurrent(
  ...
  async ({ helper, expect, stHelper }) => {
    ...
    assertJson(bSpecRes);
    const bSpec = bSpecRes.content as PColumnSpec;
    expect(bSpec.axesSpec).toMatchObject([inputSpec.axesSpec[1], xsvSettings.axes[0].spec]);
    expect(bSpec).toMatchObject(xsvSettings.columns[0].spec);
    expect(bSpec.annotations).toHaveProperty(Annotation.Trace);
  },
);
...
eTplTest.concurrent(
  ...
  async ({ helper, expect, stHelper }) => {
    ...
    assertJson(bSpecRes);
    const bSpec = bSpecRes.content as PColumnSpec;
    expect(bSpec.axesSpec).toMatchObject([inputSpec.axesSpec[0], xsvSettings.axes[0].spec]);
    expect(bSpec).toMatchObject(xsvSettings.columns[0].spec);
    expect(bSpec.annotations).toHaveProperty(Annotation.Trace);
  },
);
