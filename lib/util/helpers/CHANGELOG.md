...
- 3314d0c: Release the nameable string-key `Branded` fix. The brand key was changed from a
...
ColumnDiscoveredId | ColumnOverriddenId`) flow into an inferred exported type
  fails declaration emit with TS4023 ("has or is using name '\_\_brand' ... but
