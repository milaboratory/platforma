...
def sum_blob_length(arr: pa.Array) -> int:
    """SUM(LENGTH(CAST(col AS BLOB))) for an Arrow array, nulls contribute 0."""
    t = arr.type
    if pa.types.is_string(t) or pa.types.is_large_string(t) \
            or pa.types.is_binary(t) or pa.types.is_large_binary(t):
        s = pc.sum(pc.binary_length(arr)).as_py()
        return s or 0
...
def get_number_of_rows_in_file(conn: duckdb.DuckDBPyConnection, path: str) -> int:
    result = conn.execute("""
        SELECT num_rows
    ...
    def write_partitions_with_bloom_filters(self, intermediate_parquet: str):
        try:
            ...
            def write_part(data_file: str, partition_total_rows: int, row=None) -> Dict[str, DataInfoPart]:
                ...
                if row is not None:
                    where_conditions = [f"{axis_identifiers[i]} = ?" for i in range(len(row))]
                ...
                # Polars sorts with nulls_last=False above, so declare
                ...
                bloom_filter_options = {
                    name: {'ndv': max(partition_total_rows, 1), 'fpp': 0.05}
                    for name in schema_names
                }
                # Accumulators folded into the streaming loop: row count, per-column
                # byte size (data-only — equivalent to SUM(LENGTH(CAST(col AS BLOB)))
                # in DuckDB, stable across parquet encoders), and per-(axis-set,
                # value-column) sha256 digests for `data_digest`. Doing this here
                # replaces three post-write passes: one DuckDB COUNT, one polars
                # scan for hashes, and one DuckDB scan for byte sizes.
                value_columns = [c.column for c in self.columns]
                axes_hasher = hashlib.sha256("~".join([axis.type for axis in axes_info]).encode())
                column_hashers = {c.column: hashlib.sha256(c.type.encode()) for c in self.columns}
                column_bytes = {name: 0 for name in schema_names}
                nrows = 0
                ...
                with pq.ParquetWriter(
                    ...
                    for batch in reader:
                        ...
                        for name in schema_names:
                            column_bytes[name] += sum_blob_length(batch.column(name))
                        
                        batch_df = pl.from_arrow(batch)
                        update_hasher_with_row_hashes(
                            axes_hasher,
                            batch_df,
                        ...
                        for column_name in value_columns:
                            update_hasher_with_row_hashes(
                                column_hashers[column_name], batch_df, pl.col(column_name)
                            )

                axes_hash = axes_hasher.hexdigest()
                axes_nbytes = [column_bytes[axis.id] for axis in axes_info]
                parts: Dict[str, DataInfoPart] = {}
                for column in self.columns:
                    column_hash = column_hashers[column.column].hexdigest()
                    data_digest = hashlib.sha256(f"{axes_hash}{column_hash}".encode()).hexdigest()
                    parts[column.column] = DataInfoPart(
                        data=data_file,
                        axes=axes_info,
                        ...
                        stats=Stats(
                            ...
                            number_of_bytes=NumberOfBytes(
                                axes=axes_nbytes,
                                column=column_bytes[column.column],
                            ),
                        ),
                    )
                return parts
            ...
            if self.partition_key_length > 0:
                ...
                partition_counts = (
                    pl.read_parquet(intermediate_parquet)
                      .group_by(partitioned_axes)
                      .len()
                )
                for i, row in enumerate(partition_counts.iter_rows()):
                    key_values = row[:-1]
                ...
                partition_total_rows = get_number_of_rows_in_file(duckdb_conn, intermediate_parquet)
