...
async function generateSCSS() {
  ...
  for (const [size, lst] of Object.entries(dict)) {
    ...
    await fp.writeFile(`./src/lib/assets/icons/icons-${size}-generated.scss`, `$icons${size}: (\n${lines.join(',\n')}\n)`);
    await fp.writeFile(`./src/lib/assets/icons/icons-${size}-generated.json`, JSON.stringify(json, null, 2));
  }
}
