...
function updateSelected() {
  data.activeOption = tap(filteredOptionsRef.value.findIndex(o => o.value === props.modelValue), v => v < 0 ? 0 : v);
}
...
const textValue = computed(() => {
  ...
  return props.options.filter(o => selectedValues.includes(o.value)).map(o => o.text).join(', ');
});
...
const filteredOptionsRef = computed(() => {
  ...
  return (data.search ? props.options.filter(opt => {
  ...
  >
    ...
    <div class="ui-multi-dropdown__container">
      ...
      <div v-if="data.open" ref="list" class="ui-multi-dropdown__options">
        ...
        <div v-if="!filteredOptionsRef.length" class="nothing-found">
          Nothing found
        </div>
      </div>
