...
test("check runUploadFile", async () => {
  const { plEndpoint, plUser, plPassword } = testCredentials();
  const { logger, lsDriver, uploadBlobClient, client, signer, terminate } = await initNetworkCheck(
    plEndpoint,
    plUser,
    plPassword,
  );
  ...
  const blob = await runUploadFile(logger, signer, lsDriver, uploadBlobClient, client, filePath);
...
test("check runDownloadFile", async () => {
  const { plEndpoint, plUser, plPassword } = testCredentials();
  const { logger, lsDriver, uploadBlobClient, downloadClient, client, terminate } =
...
test("check downloadFromEveryStorage", async () => {
  const { plEndpoint, plUser, plPassword } = testCredentials();
  const { logger, client, lsDriver, terminate } = await initNetworkCheck(
    plEndpoint,
    plUser,
    plPassword,
  );
