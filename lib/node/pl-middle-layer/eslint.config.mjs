...
export default [
  { ignores: ['src/**/*.test.ts', 'jest.config.js', 'build.node.config.mjs'] },
  ...node,
  {
    rules: {
      /**
      ...
      /**
       * IMPORTANCE: MEDIUM
       * Prevents using template expressions with non-string values.
       * This rule ensures that template expressions are used with strings only.
       * Currently disabled to support legacy code, but should be addressed systematically.
       */
