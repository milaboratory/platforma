...
describe("SSH Tests", () => {
  it("isPassphraseRequiredForKey", async () => {
    ...
    const k3 = ssh.utils.generateKeyPairSync("ecdsa", {
      bits: 256,
    ...
    const k4 = ssh.utils.generateKeyPairSync("ecdsa", {
      bits: 256,
      comment: "node.js rules!",
      cipher: "aes256-cbc",
    });
  ...
  it("Upload directory", async () => {
    ...
    const execResult1 = await testContainer.exec([
      "cat",
      `${remoteDir}/rec-upload/sub-1/sub-1-1/test-5.txt`,
    ]);
    const output1 = execResult1.output.trim();
    expect(output1).toBe("test-5");

    const execResult2 = await testContainer.exec([
      "cat",
      `${remoteDir}/rec-upload/sub-1/sub-1-18/test-2.txt`,
    ]);
  ...
  it("Simple server should forward remote SSH port to a local port", async () => {
    ...
    const resFailed = await fetch(`http://127.0.0.1:${localPort}`).catch((err) =>
...
describe("sshConnect", () => {
  it("should successfully connect to the SSH server", async () => {
    const client = await SshClient.init(
      new ConsoleLoggerAdapter(),
      getConnectionForSsh(testContainer),
    );
    client.close();
  });
  ...
  it("should fail with invalid credentials", async () => {
    await expect(
      SshClient.init(new ConsoleLoggerAdapter(), {
        ...getConnectionForSsh(testContainer),
        username: "dfasdfa",
      }),
    ).rejects.toThrow();
  ...
  it.skip("should fail with invalid passphrase", async () => {
    let catched = false;
    try {
      await SshClient.init(new ConsoleLoggerAdapter(), {
        ...getConnectionForSsh(testContainer),
        passphrase: "dfasdfa",
      });
    } catch (e) {
      ...
      expect(
        (e as Error).message.includes(
          "Cannot parse privateKey: OpenSSH key integrity check failed",
        ),
      ).toBe(true);
      catched = true;
    }
  ...
  it("should fail with invalid username", async () => {
    ...
    try {
      ...
      console.log(e);
      expect((e as Error).message.includes("All configured authentication methods failed")).toBe(
        true,
      );
      catched = true;
    }
  ...
  it("should fail with invalid private key", async () => {
    ...
    const invalidPk = ssh.utils.generateKeyPairSync("ecdsa", {
      bits: 256,
      comment: "node.js rules!",
      passphrase: "password",
      cipher: "aes256-cbc",
  ...
  it("should timeout if the server is unreachable", async () => {
    ...
    ).rejects.toThrow("ssh.connect: AggregateError");
  });
});
