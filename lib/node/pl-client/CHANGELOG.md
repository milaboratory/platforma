...
- 8f77fbc: Raise the HTTP/2 connection window to 16 MiB via `grpc-node.flow_control_window`, and bump `@grpc/grpc-js` to 1.14.4 where that option drives the connection-level window and not just the per-stream one. On a high-latency link a large project open was capped at window/RTT instead of link speed.
...
- effca5f: Fix connecting to old backends that lack the sharing RPCs (MILAB-2668).
...
- de415f7: Sync pl proto API: resource signing, access control, locks, auth, field renames.
...
- ba792d4: Disable gRPC DNS TXT record lookups for service config discovery.
