...
async function runCommand(commandAndArgs, options) {
  try {
    ...
    console.error(`Command "${commandAndArgs.join(" ")}" failed: ${error.message}`);
...
async function createSparseFile(platform, currentFilePath, size) {
  try {
    ...
    if (platform === "win32") {
      console.log(`Windows: File "${currentFilePath}" created/truncated to ${size} bytes.`);
    ...
    await fileHandle.write(distantData, 0, distantData.length, distantOffset);
...
(async () => {
  ...
  const platformFilePath = path.join(testDir, `${currentPlatform}-${baseFileName}`);
  ...
  if (currentPlatform === "win32") {
    const queryFlag = await runCommand(["fsutil", "sparse", "queryflag", `"${platformFilePath}"`], {
      stdio: "pipe",
    });
    console.log(
      `Windows: File "${platformFilePath}" is sparse: ${queryFlag.stdout}, error: ${queryFlag.stderr}`,
    );

    const queryRange = await runCommand(
      ["fsutil", "sparse", "queryrange", `"${platformFilePath}"`],
      { stdio: "pipe" },
    );
    console.log(
      `Windows: Query range for "${platformFilePath}": ${queryRange.stdout}, error: ${queryRange.stderr}`,
    );
  }
})();
