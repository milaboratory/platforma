...
export async function getLicenseFromEnv(): Promise<License> {
  ...
  if (licenseFile !== undefined) return { type: 'file', file: licenseFile };

  throw new Error('no license in envs');
}

export function mergeLicense(license: License, config: any) {
  if (license.type == 'value') config['license']['value'] = license.value;
  else if (license.type == 'file') config['license']['file'] = license.file;
  else assertNever(license);
}
